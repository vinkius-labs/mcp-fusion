...
import { type ToolBuilder, type InternalAction } from '../types.js';
...
function getBuilderActions<TContext>(
    ...
    if (typeof b.getActions === 'function') {
        return b.getActions();
    }
    return undefined;
}
 ...
 */
function getBuilderDiscriminator(builder: ToolBuilder<unknown>): string {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const b = builder as any;
    if (typeof b.getDiscriminator === 'function') {
        return b.getDiscriminator();
    }
    return 'action';
}
 ...
 */
function getBuilderCommonSchema(builder: ToolBuilder<unknown>): ZodObject<ZodRawShape> | undefined {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const b = builder as any;
    if (typeof b.getCommonSchema === 'function') {
        return b.getCommonSchema();
    }
    return undefined;
}
