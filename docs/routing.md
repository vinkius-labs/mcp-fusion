...
MCP Fusion separates how you author tools from how the agent discovers them. You organize tools as files in a directory tree; the framework maps that tree into MCP tool definitions with clear naming, discriminators, and shared schemas. This works for hand-written tools and for code generated by [@vinkius-core/mcp-fusion-openapi-gen](/openapi-gen) or [@vinkius-core/mcp-fusion-prisma-gen](/prisma-gen) — drop generated files into `src/tools/` and `autoDiscover()` picks them up.
